...
export default function App() {
  ...
  async function replayAction(action: "start" | "pause" | "reset" | "step") { if (!mission) return; try { const state = await missions.replay(mission.mission_id, action, speed); setEventCursor(state.cursor); setPlaying(state.playing); } catch (e) { setError(e instanceof Error ? e.message : "Replay control failed"); } }
  ...
  return <main className="app"><header><div><p className="eyebrow">VANRAKSHAK // C2 CONTROL</p><h1>Forest mission console</h1></div><span className="status">● SYSTEM ONLINE</span></header>
    ...
    <section className="panel"><div className="panel-title">TELEMETRY</div><div className="metric"><span>BATTERY</span><strong>{mission?.telemetry.battery_pct ?? "—"}%</strong></div><div className="metric"><span>WIND</span><strong>{mission?.telemetry.wind_mps ?? "—"} m/s</strong></div><div className="metric"><span>GPS</span><strong>{mission?.telemetry.gps ? `${Number(mission.telemetry.gps.lat).toFixed(4)}, ${Number(mission.telemetry.gps.lng).toFixed(4)}` : "—"}</strong></div></section>
    {mission?.telemetry_history.length ? <section className="panel"><div className="panel-title">TELEMETRY TIMELINE</div><div className="stream">{mission.telemetry_history.map((sample, index) => <div className="event" key={index}><span>{Number(sample.timestamp_seconds).toFixed(1)}s</span><span>BAT {String(sample.battery_pct)}% // WIND {String(sample.wind_mps)} m/s</span></div>)}</div></section> : null}
    <section className="panel"><div className="panel-title"><span>MISSION EVENT STREAM</span><span className="badge">{eventCursor}/{events.length}</span></div><div className="replay-controls"><button disabled={!events.length} onClick={() => replayAction(playing ? "pause" : "start")}>{playing ? "PAUSE" : "START"}</button><button disabled={!events.length} onClick={() => replayAction("reset")}>RESET</button><button disabled={eventCursor >= events.length} onClick={() => replayAction("step")}>STEP</button><label>SPEED <select value={speed} onChange={(event) => setSpeed(Number(event.target.value))}><option value={0.5}>0.5×</option><option value={1}>1×</option><option value={2}>2×</option></select></label></div>{events.length ? <div className="stream">{events.slice(0, eventCursor).map((event) => <button className="event" key={event.sequence} onClick={() => setSelectedEvent(event)}><span>{event.timestamp_seconds.toFixed(2)}s</span><span>{event.type}{event.track_id ? ` // TRACK ${event.track_id}` : ""}</span></button>)}</div> : <p className="muted">No mission events yet.</p>}{selectedEvent && <div className="evidence"><div className="panel-title"><span>EVIDENCE DRAWER</span><button onClick={() => setSelectedEvent(undefined)}>CLOSE</button></div><p>{selectedEvent.type} // sequence {selectedEvent.sequence}</p><p className="muted">Evidence: {selectedEvent.evidence_refs.join(", ") || "none"}</p><pre>{JSON.stringify(selectedEvent.payload, null, 2)}</pre></div>}</section>
