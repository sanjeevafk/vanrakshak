...
export default function App() {
  ...
  async function analyze() { if (!file) return; setBusy(true); setError(undefined); try { const next = await vision.detectVideo(file); setResult(next); if (next.representative_frame) setSceneResult(await scene.understand(next.representative_frame)); } catch (e) { setError(e instanceof Error ? e.message : "Analysis failed"); } finally { setBusy(false); } }
